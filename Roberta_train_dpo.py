...
def dop_loss(a, p, n, reference_a, reference_p, reference_n, beta=0.1):
    cos = nn.CosineSimilarity(dim=1)

    pos_scores = cos(a, p)
    neg_scores = cos(a, n)
    reference_pos_scores = cos(reference_a, reference_p)
    reference_neg_scores = cos(reference_a, reference_n)

    pos_ratios = pos_scores / (reference_pos_scores + 1e-8)
    neg_ratios = neg_scores / (reference_neg_scores + 1e-8)
...
reference_model = AutoModel.from_pretrained('BM-K/KoSimCSE-roberta-multitask', cache_dir=cache_dir).to(device)
checkpoint = torch.load('path_to_save/roberta_contrastive_checkpoint_epoch_1.pth')
...
for epoch in range(3):  # Number of epochs
    ...
    for batch in tqdm(train_loader):
        ...
        reference_anchor_embeddings = reference_model(batch['anchor_input_ids'], batch['anchor_attention_mask'])
        reference_positive_embeddings = reference_model(batch['positive_input_ids'], batch['positive_attention_mask'])
        reference_negative_embeddings = reference_model(batch['negative_input_ids'], batch['negative_attention_mask'])

        loss = dop_loss(anchor_embeddings, positive_embeddings, negative_embeddings)
